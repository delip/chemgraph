import json
import argparse
from chemgraph.tools.ase_tools import run_ase, save_atomsdata_to_file
...
def get_saved_file_from_smiles(
    ...
    try:
        ...
        filepath = os.path.join(output_path, smiles + '.xyz')
...
def main(fname: str, n_structures: int):
    ...
    for idx, molecule in enumerate(smiles_data[:n_structures]):
        ...
        try:
            ...
            manual_workflow = get_saved_file_from_smiles(
                smiles,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run geometry optimization and save the optimized structure from a SMILES string."
    )
    parser.add_argument(
        "--fname",
        type=str,
